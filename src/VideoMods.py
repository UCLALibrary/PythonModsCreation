...
def main():
    with open(r'\\svm-netapp-dlib.in.library.ucla.edu\DLIngest\gm_new_videos\gm_new_videos_metadata.csv',
...
def printxml(row):
    ...
    with open(r'\\svm-netapp-dlib.in.library.ucla.edu\DLIngest\gm_new_videos\mods\{}.xml'.format(row['filename'].strip()), 'w', encoding='utf-8') as f:
